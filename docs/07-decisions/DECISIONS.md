...
- **D-P0.7 — Deferred from this P0 pass (per scope decision, 2026-06-06):** 0.0a security headers + CSP nonce, 0.4a privacy/PII scrubber + consent store, 0.14a supply-chain CI (Renovate/Lighthouse/SBOM/license-checker), 0.8a intercepting-routes spike. The core **runnable, team-ready skeleton + design system** is complete (`build`/`tsc`/`lint`/`test` green). These deferred blocks must close before the formal P0 acceptance checklist is fully ticked.
