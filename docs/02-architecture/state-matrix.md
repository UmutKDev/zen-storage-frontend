...
- **Inside the page**, the feature handles empty / no‑results / locked / reveal‑required / quota / permission.
- Each [feature spec](../04-features/FEATURE-MAP.md) lists *which* of these states apply to *that* surface (not all
  apply everywhere — e.g. quota applies to uploads, not the account page).
