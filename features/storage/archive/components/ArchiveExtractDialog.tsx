...
export function ArchiveExtractDialog({
  ...
  const { preview, start, starting } = useArchiveExtract(archiveKey, path, open);
  ...
  const onExtract = async () => {
    const ok = await start({
      // Full extraction omits the list entirely; a subset sends the chosen paths.
      selectedEntries: allSelected ? undefined : [...selected],
      strategy,
      createFolder,
    });
    if (!ok) return;
